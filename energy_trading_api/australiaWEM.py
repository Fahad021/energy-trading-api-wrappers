...
def participants():
    return __call_api(endpoint="participants/participants.csv")

def facilities():
    return __call_api(endpoint="facilities/facilities.csv")

def commissioningTest(year="2019"):
    return __call_api(endpoint=f"commissioning-test/commissioning-test-summary-{year}.csv")

def loadForecast():
    return __call_api(endpoint="load-forecast/load-forecast.csv")

def extendedLoadForecast():
    return __call_api(endpoint="extended-load-forecast/extended-load-forecast.csv")
...
def ircrRatios():
    return __call_api(endpoint="ircr-ratios/ircr-ratios.csv")

def peakSWISIntervals():
    return __call_api(endpoint="peak-intervals/peak-intervals.csv")

def realTimeOutages():
    return __call_api(
        endpoint="realtime-outages/realtime-outages.csv", ignore_errors=True
    )

#REPO = refund exempt planned outage count
def repoCount():
    return __call_api(endpoint="repo-count/repo-count.csv")

def dispatchAdvisory():
    return __call_api(
        endpoint="dispatch-advisory/dispatch-advisory.csv", ignore_errors=True
    )

def marketAdvisory():
    return __call_api(endpoint="market-advisory/market-advisory.csv")
